...
for i,fi in enumerate(infiles):
    ...
    if ifl:           
        # with open(ifl) as file: 
        # lines = ifi.readlines()
        if "nfs" not in ifl[1]:
            lineone = ifl[0].replace("starting dir:","").replace('\n', '')
            propername = lineone+"/"+ifl[1].replace('\n', '')
        else:
            propername = ifl[1].replace('\n', '')
        # print(propername)
        # propername = (ifl[0].replace("starting dir:","")+"/"+ifl[1]).lstrip().replace('\n', '')
    else:
        print("bkdbwhlbdw")
        propername=fi.replace(os.path.abspath(indir)+"/","").split(".o")[0]
    ...
    if treeWasWritten==False or anyProblem==True:
        ...
        problematicLogFileList.append(fi)
...
with open("problematicLogFiles.txt","w")as f:
    f.write("\n".join(problematicLogFileList))
