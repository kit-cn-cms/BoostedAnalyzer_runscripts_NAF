...
directory = "/nfs/dust/cms/user/mhorzela/ttbb_data/ntuples_scalefac/"
...
############################################################
sample_dict = {}
with open("../samplelists/samples_mergedttbb.csv", "rb") as f:
    reader = csv.reader(f)
    for row in reader:
...
for i in range(len(samples)):
    ...
    for branch in branches:
        branch_name = branch
        print (branch_name)
        if branch_name.find("muR") != -1 or branch_name.find("muF") != -1 or branch_name.find("LHA_306000") != -1 or branch_name.find("LHA_320900") != -1:
            # canvas=ROOT.TCanvas()
            print ("!!!!!!!!!", branch_name, "!!!!!!!!!!")
            file_.Draw(
